...
cursor.execute("""
    CREATE TABLE IF NOT EXISTS attendance (
        name TEXT, time TEXT, date TEXT, subject TEXT, 
...
class FaceRecognizer:
    ...
    def mark_attendance(self, name):
        ...
        cursor.execute("SELECT 1 FROM attendance WHERE name = ? AND date = ? AND subject = ?",
                       (name, current_date, self.subject))
        already_marked = cursor.fetchone()

        if not already_marked:
            cursor.execute("INSERT INTO attendance (name, time, date, subject) VALUES (?, ?, ?, ?)",
                           (name, current_time, current_date, self.subject))
            conn.commit()
        ...
        for (x, y, w, h) in faces:
            ...
            if hasattr(self.recognizer, 'predict'):
                try:
                    ...
                    name = self.label_map[label] if confidence < 50 and label in self.label_map else "Unknown"
                except:
                    name = "Unknown"
            else:
